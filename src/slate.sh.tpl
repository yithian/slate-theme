...
# Writes the Slate GNOME Terminal theme colors and configurations as dconf key-value pairs to the target profile.
#
...
apply() {
  local \
    ...
    slate15="<%= slate15 %>"
  local \
    slate0_rgb="rgb(<%= parseInt(slate0.substr(1,2), 16) %>,<%= parseInt(slate0.substr(3,2), 16) %>,<%= parseInt(slate0.substr(5,2), 16) %>)"
    slate1_rgb="rgb(<%= parseInt(slate1.substr(1,2), 16) %>,<%= parseInt(slate1.substr(3,2), 16) %>,<%= parseInt(slate1.substr(5,2), 16) %>)"
    slate4_rgb="rgb(<%= parseInt(slate4.substr(1,2), 16) %>,<%= parseInt(slate4.substr(3,2), 16) %>,<%= parseInt(slate4.substr(5,2), 16) %>)"
    slate8_rgb="rgb(<%= parseInt(slate8.substr(1,2), 16) %>,<%= parseInt(slate8.substr(3,2), 16) %>,<%= parseInt(slate8.substr(5,2), 16) %>)"

  _write palette "['$slate1', '$slate11', '$slate14', '$slate13', '$slate9', '$slate15', '$slate8', '$slate5', '$slate3', '$slate11', '$slate14', '$slate13', '$slate9', '$slate15', '$slate7', '$slate6']"
  log 4 "Applied Slate color palette"

  _write background-color "'$slate0'"
  _write foreground-color "'$slate4'"
  _write use-transparent-background "false"
  log 4 "Applied background- and foreground colors"

  _write bold-color "'$slate4'"
  ...
  _write cursor-foreground-color "'$slate1_rgb'"
  _write cursor-background-color "'$slate4_rgb'"
  log 4 "Applied cursor colors and configuration"

  _write highlight-colors-set "true"
  _write highlight-foreground-color "'$slate0_rgb'"
  _write highlight-background-color "'$slate8_rgb'"
  ...
  _write "$SLATE_GNOME_TERMINAL_VERSION_DCONF_KEY" "'$SLATE_GNOME_TERMINAL_VERSION'"
  log 4 "Set Slate GNOME Terminal version key of the '$profile_name' profile"

  log 3 "Applied theme colors and configurations"
}

# Applies the Slate GNOME Terminal theme with version comparison.
#
...
apply_version_compared() {
  local version
  version=$1
  case "$(vercomp "$SLATE_GNOME_TERMINAL_VERSION" "$version"; echo $?)" in
    0)
      log 3 "Reinstalling Slate GNOME Terminal since the version equaled the version of the '$profile_name' profile!";
      apply;
      log 2 "Reinstalled Slate GNOME Terminal version $SLATE_GNOME_TERMINAL_VERSION for the '$profile_name' profile!";
      exit 0;;
    1)
      log 4 "The script version is newer than the currently installed theme ($SLATE_GNOME_TERMINAL_VERSION > $version)"
      apply;
      log 2 "The '$profile_name' profile has been updated successfully from version $version to $SLATE_GNOME_TERMINAL_VERSION";
      exit 0;;
    2)
      log 1 "The detected Slate GNOME Terminal version $version of the '$profile_name' profile is greater than the version that will be applied!";
      printf "${_ctb}> [?]${_ct} Override current profile version %s with script version %s? (${_ctb}y${_cr}/${_ctb}n${_cr})" "$version" "$SLATE_GNOME_TERMINAL_VERSION"
      read -r -n 1 -s confirmation;
      echo
      case $confirmation in
        [Yy]* )
          apply;
          log 2 "Slate GNOME Terminal version $SLATE_GNOME_TERMINAL_VERSION has been successfully applied!";
          exit 0;;
        [Nn]* )
...
cleanup() {
  ...
  unset -v SLATE_GNOME_TERMINAL_SCRIPT_OPTS SLATE_GNOME_TERMINAL_VERSION SLATE_GNOME_TERMINAL_VERSION_DCONF_KEY SLATE_PROFILE_VISIBLE_NAME log_level DEPENDENCIES DCONF_PROFILE_BASE_PATH GSETTINGS_PROFILELIST_PATH gnome_terminal_version profile_name profile_uuid
...
clone_default_profile() {
  ...
  dconf write "$DCONF_PROFILE_BASE_PATH"/:"$profile_uuid"/visible-name "'$SLATE_PROFILE_VISIBLE_NAME'"
  append_profile_uuid_to_list "$profile_uuid"
  log 3 "Cloned the default profile '$uuid' with new UUID '$profile_uuid'"
}

# Gets the current Slate GNOME Terminal version.
#
# @globread SLATE_GNOME_TERMINAL_VERSION_DCONF_KEY DCONF_PROFILE_BASE_PATH
...
get_current_version() {
  ...
  uuid=$1
  version=
  printf "$(dconf read $DCONF_PROFILE_BASE_PATH/:"$uuid"/$SLATE_GNOME_TERMINAL_VERSION_DCONF_KEY | tr -d "'")"
  return 0
}
...
SLATE_GNOME_TERMINAL_SCRIPT_OPTS=$(getopt -o hl:p: --long help,loglevel:,profile: -n 'slate.sh' -- "$@")
SLATE_GNOME_TERMINAL_VERSION=0.1.0
SLATE_GNOME_TERMINAL_VERSION_DCONF_KEY=slate-gnome-terminal-version
SLATE_PROFILE_VISIBLE_NAME="Slate"
log_level=2
...
eval set -- "$SLATE_GNOME_TERMINAL_SCRIPT_OPTS"
while true; do
  case "$1" in
...
if validate_dependencies DEPENDENCIES[@]; then
  ...
  if [[ -n $profile_name ]]; then
    ...
    if [[ -n $profile_uuid ]]; then
      ...
      if [[ -n $curr_ver ]]; then
        ...
        apply
        log 2 "Slate GNOME Terminal version $SLATE_GNOME_TERMINAL_VERSION has been successfully applied to the '$profile_name' profile"
        exit 0
      fi
    else
  ...
  # Check for an already existing 'Slate' profile and update it
  profile_uuid="$(get_profile_uuid_by_name $SLATE_PROFILE_VISIBLE_NAME)"
  if [[ -n $profile_uuid ]]; then
    profile_name="$SLATE_PROFILE_VISIBLE_NAME"
    log 4 "Found already existing '$SLATE_PROFILE_VISIBLE_NAME' profile with UUID '$profile_uuid'"
    log "Updating already existing '$SLATE_PROFILE_VISIBLE_NAME' profile"
    apply_version_compared "$(get_current_version $profile_uuid)"
  fi

  clone_default_profile
  apply
  log 2 "Slate GNOME Terminal version $SLATE_GNOME_TERMINAL_VERSION has been successfully applied to the newly created '$SLATE_PROFILE_VISIBLE_NAME' profile"
  exit 0
else
