...
apply_version_compared() {
  ...
  case "$(vercomp "$NORD_GNOME_TERMINAL_VERSION" "$version"; echo $?)" in
    0)
      ...
      apply;
      log 2 "Reinstalled Slate GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION for the '$profile_name' profile!";
      exit 0;;
    1)
    ...
    2)
      ...
      case $confirmation in
        [Yy]* )
          apply;
          log 2 "Slate GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION has been successfully applied!";
          exit 0;;
        [Nn]* )
...
NORD_GNOME_TERMINAL_SCRIPT_OPTS=$(getopt -o hl:p: --long help,loglevel:,profile: -n 'slate.sh' -- "$@")
NORD_GNOME_TERMINAL_VERSION=0.1.0
NORD_GNOME_TERMINAL_VERSION_DCONF_KEY=slate-gnome-terminal-version
NORD_PROFILE_VISIBLE_NAME="Slate"
log_level=2
...
if validate_dependencies DEPENDENCIES[@]; then
  ...
  if [[ -n $profile_name ]]; then
    ...
    if [[ -n $profile_uuid ]]; then
      ...
      if [[ -n $curr_ver ]]; then
        ...
        apply
        log 2 "Slate GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION has been successfully applied to the '$profile_name' profile"
        exit 0
      fi
    else
  ...
  clone_default_profile
  apply
  log 2 "Slate GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION has been successfully applied to the newly created '$NORD_PROFILE_VISIBLE_NAME' profile"
  exit 0
else
